        ...
        <div :id="`${caseId}tbody`" class="tbody" :style="tbodyStyle">
          ...
          <draggable :value="rows" handle=".td-ds-icons" @start="handleDragMoveStart" @end="handleDragMoveEnd">
            ...
            <template v-for="(row,rowIndex) in rows">
              ...
              "
                :id="`${caseId}tbody-tr-${rowIndex}`" :data-idx="rowIndex" class="tr" :class="selectedRowIds.indexOf(row.id) !== -1 ? 'tr-checked' : ''"
                :style="buildTrStyle(rowIndex)" :key="row.id">
                <!-- 左侧固定td  -->
                ...
                <div class="td" v-for="col in columns" v-show="col.type !== formTypes.hidden" :key="col.key" :style="buildTdStyle(col)">
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <label :key="i" v-if="col.type === formTypes.input || col.type === formTypes.inputNumber">
                      <a-tooltip :id="id" placement="top" :title="(tooltips[id] || {}).title" :visible="(tooltips[id] || {}).visible || false"
                        :autoAdjustOverflow="true" :getPopupContainer="getParentContainer">

                        <input :id="id" v-bind="buildProps(row,col)" :data-input-number="col.type === formTypes.inputNumber"
                          :placeholder="replaceProps(col, col.placeholder)" @blur="(e)=>{handleBlurCommono(e.target,rowIndex,row,col)}"
                          @input="(e)=>{handleInputCommono(e.target,rowIndex,row,col)}" @mouseover="()=>{handleMouseoverCommono(row,col)}"
                    ...
                    <template v-else-if="col.type === formTypes.select">
                      <a-tooltip :key="i" :id="id" placement="top" :title="(tooltips[id] || {}).title" :visible="(tooltips[id] || {}).visible || false"
                        :autoAdjustOverflow="true" :getPopupContainer="getParentContainer">

                        <span @mouseover="()=>{handleMouseoverCommono(row,col)}" @mouseout="()=>{handleMouseoutCommono(row,col)}">

                          <a-select :id="id" :key="i" v-bind="buildProps(row,col)" style="width: 100%;" :value="selectValues[id]"
                            :options="col.options" :getPopupContainer="getParentContainer" :placeholder="replaceProps(col, col.placeholder)"
                            :filterOption="(i,o)=>handleSelectFilterOption(i,o,col)" @change="(v)=>handleChangeSelectCommon(v,id,row,col)"
                            @search="(v)=>handleSearchSelect(v,id,row,col)" @blur="(v)=>handleBlurSearch(v,id,row,col)"
                            @focus="(v)=>handledropdownVisibleFocus(v,id,row,col)" allowClear showSearch>
                    ...
                    <template v-else-if="col.type === formTypes.date || col.type === formTypes.datetime">
                      <a-tooltip :key="i" :id="id" placement="top" :title="(tooltips[id] || {}).title" :visible="(tooltips[id] || {}).visible || false"
                        ...
                        <span @mouseover="()=>{handleMouseoverCommono(row,col)}" @mouseout="()=>{handleMouseoutCommono(row,col)}">
                          ...
                          <j-date :id="id" :key="i" v-bind="buildProps(row,col)" style="width: 100%;" :value="jdateValues[id]"
                            :getCalendarContainer="getParentContainer" :placeholder="replaceProps(col, col.placeholder)"
                            :trigger-change="true" :showTime="col.type === formTypes.datetime" :dateFormat="col.type === formTypes.date? 'YYYY-MM-DD':'YYYY-MM-DD HH:mm:ss'"
                            allowClear @change="(v)=>handleChangeJDateCommon(v,id,row,col,col.type === formTypes.datetime)" />
                    ...
                    <template v-else-if="col.type === formTypes.input_pop">
                      <a-tooltip :key="i" :id="id" placement="top" :title="(tooltips[id] || {}).title" :visible="(tooltips[id] || {}).visible || false"
                        :autoAdjustOverflow="true" :getPopupContainer="getParentContainer">
                        <span @mouseover="()=>{handleMouseoverCommono(row,col)}" @mouseout="()=>{handleMouseoutCommono(row,col)}">
                          <j-input-pop :id="id" :key="i" :width="300" :height="210" :pop-container="`${caseId}tbody`"
                            v-bind="buildProps(row,col)" style="width: 100%;" :value="jInputPopValues[id]"
                            :getCalendarContainer="getParentContainer" :placeholder="replaceProps(col, col.placeholder)"
                    ...
                    <div v-else-if="col.type === formTypes.upload" :key="i">
                      ...
                      <div :hidden="uploadValues[id] != null">
                        <a-tooltip :key="i" :id="id" placement="top" :title="(tooltips[id] || {}).title" :visible="(tooltips[id] || {}).visible || false"
                          ...
                          <span @mouseover="()=>{handleMouseoverCommono(row,col)}" @mouseout="()=>{handleMouseoutCommono(row,col)}">
                            <a-upload name="file" :data="{'isup':1}" :multiple="false" :action="col.action" :headers="uploadGetHeaders(row,col)"
                              :showUploadList="false" v-bind="buildProps(row,col)" @change="(v)=>handleChangeUpload(v,id,row,col)">
                    ...
                    <template v-else-if="col.type === formTypes.popup">
                      <a-tooltip :key="i" :id="id" placement="top" :title="(tooltips[id] || {}).title" :visible="(tooltips[id] || {}).visible || false"
                        ...
                        <span @mouseover="()=>{handleMouseoverCommono(row,col)}" @mouseout="()=>{handleMouseoutCommono(row,col)}">
                          ...
                          <j-popup :id="id" :key="i" v-bind="buildProps(row,col)" :placeholder="replaceProps(col, col.placeholder)"
                            style="width: 100%;" :value="getPopupValue(id)" :field="col.field || col.key" :org-fields="col.orgFields"
                            :dest-fields="col.destFields" :code="col.popupCode" :groupId="caseId" @input="(value,others)=>popupCallback(value,others,id,row,col,rowIndex)" />
                        </span>
                      </a-tooltip>
                    </template>
                    ...
                    <div v-else-if="col.type === formTypes.file" :key="i">
                      ...
                      <div :hidden="uploadValues[id] != null">
                        <a-tooltip :key="i" :id="id" placement="top" :title="(tooltips[id] || {}).title" :visible="(tooltips[id] || {}).visible || false"
                          ...
                          <span @mouseover="()=>{handleMouseoverCommono(row,col)}" @mouseout="()=>{handleMouseoutCommono(row,col)}">
                            <a-upload name="file" :data="{'isup':1}" :multiple="false" :action="getUploadAction(col.action)"
                              :headers="uploadGetHeaders(row,col)" :showUploadList="false" v-bind="buildProps(row,col)"
                    ...
                    <template v-else-if="col.type === formTypes.list_multi">
                      <a-tooltip :key="i" :id="id" placement="top" :title="(tooltips[id] || {}).title" :visible="(tooltips[id] || {}).visible || false"
                        ...
                        <span @mouseover="()=>{handleMouseoverCommono(row,col)}" @mouseout="()=>{handleMouseoutCommono(row,col)}">
                          ...
                          <a-select :id="id" :key="i" mode="multiple" :maxTagCount="1" v-bind="buildProps(row,col)"
                            style="width: 100%;" :value="multiSelectValues[id]" :options="col.options"
                    ...
                    <template v-else-if="col.type === formTypes.sel_search">
                      <a-tooltip :key="i" :id="id" placement="top" :title="(tooltips[id] || {}).title" :visible="(tooltips[id] || {}).visible || false"
                        ...
                        <span @mouseover="()=>{handleMouseoverCommono(row,col)}" @mouseout="()=>{handleMouseoutCommono(row,col)}">
                          ...
                          <a-select :id="id" :key="i" showSearch optionFilterProp="children" :filterOption="filterOption"
                            v-bind="buildProps(row,col)" style="width: 100%;" :value="searchSelectValues[id]" :options="col.options"
                            :getPopupContainer="getParentContainer" :placeholder="replaceProps(col, col.placeholder)"
                      ...
                      <a-tooltip :key="i" :id="id" placement="top" :title="(tooltips[id] || {}).title" :visible="(tooltips[id] || {}).visible || false"
                        ...
                        <span @mouseover="()=>{handleMouseoverCommono(row,col)}" @mouseout="()=>{handleMouseoutCommono(row,col)}">
                          <slot :name="(col.slot || col.slotName) || col.key" :index="rowIndex" :text="slotValues[id]"
                            :value="slotValues[id]" :column="col" :rowId="getCleanId(row.id)" :getValue="()=>_getValueForSlot(row.id)"
                            :caseId="caseId" :allValues="_getAllValuesForSlot()" :target="getVM()" :handleChange="(v)=>handleChangeSlotCommon(v,id,row,col)"
  ...
  export default {
    ...
    data() {
      return {
        ...
        statisticsColumns: {},
        // 只有在行修改被销毁时才主动清空GroupRequest的内存
        ...
        handler(columns) {
          ...
          this.getElementPromise('tbody').then(() => {
            columns.forEach(column => {
              if (column.type === FormTypes.select || column.type === FormTypes.list_multi || column.type ===
                FormTypes.sel_search) {
                // 兼容 旧版本 options
    ...
    methods: {
      ...
      _pushByDataSource(dataSource, insertIndexes = null, update = true, rows = null, setDefaultValue = false) {
        ...
        dataSource.forEach((data, newValueIndex) => {
          ...
          this.columns.forEach(column => {
            ...
            } else if (column.type === FormTypes.upload || column.type === FormTypes.file || column.type ===
              FormTypes.image) {
              if (sourceValue) {
                let fileName = ''
      ...
      /** 删除被选中的行 */
      removeSelectedRows() {
       this.$emit('bthDeleted',this.selectedRowIds,this)
        ...
        for (let value of inputValues) {
          ...
          this.columns.forEach(column => {
            ...
            if (validate === true) {
              ...
              asyncCount++
              let results = this.validateOneInputAsync(value[column.key], value, column, notPassedIds, false,
                'getValues', (results) => {
      ...
      handleBlurSearch(value, id, row, col) {
        ...
        // =============  // 下面那个
        // if((col.type === 'select') && (col.key === 'virtualdevSerialnum')){
        //   this.handledropdownVisibleChange(value,id,row,col)
        // }
      },
      // ==============    //这个 聚集事件
      handledropdownVisibleFocus(value, id, row, column) {
        if ((column.type === 'select') && (column.key === 'virtualdevSerialnum')||(column.type === 'select') && (column.key === 'invoiceterminalCode')) {
          this.elemValueFocus(FormTypes.select, row, column, value);
        }
      },
      /** 触发已拖动事件 */
      ...
      elemValueChange(type, rowSource, columnSource, value) {
        ...
        // if(type === 'select' && column.title === '设备编号'){
        //   this.$emit('xnsbValueChange', { type, row, column, value, target: this })
        // }else{
      ...
      elemValueFocus(type, rowSource, columnSource, value) {
        let column = Object.assign({}, columnSource)
        // 将caseId去除
        let row = Object.assign({}, rowSource)
        row.id = this.getCleanId(row.id)
      ...
      buildTdStyle(col) {
        const isEmptyWidth = (column) => (column.type === FormTypes.hidden || column.width === '0px' || column.width ===
          '0' || column.width === 0)
          ...
          Object.keys(others).map((key) => {
            this.columns.map(k => {
              if (k.key === key) {
                ...
                if (k.type === 'date') {
                  this.handleChangeJDateCommon(others[key], key + tempId, {
                    id: tempId
                  }, k, false)
