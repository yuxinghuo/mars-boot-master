<template>
  <a-modal :title="currTitle" :width="450" :visible="visible" :closable="false" :maskClosable="closable">
    ...
    <a-form>
      <a-form-item :labelCol="{span:4}" :wrapperCol="{span:20}" :style="{marginBottom:'10px',marginLeft: '15%'}"
        ...
        <a-select v-model="departSelected"  :class="{'valid-error':validate_status=='error'}"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      departOk() {
        console.log('11111111111111111','我是切换部门页面')
        ...
        putAction("/sys/selectDepart", obj).then(res => {
          if (res.success) {
            ...
            this.departClear();
            // this.$store.dispatch('TOSET_CHANGEDEPARTMENT',true)
            // spl修改 部门做了切换 从新把机构名称下拉框数据缓存到本地
            this.globalNameInstitution();
             // zk   2020-12-09  部门做了切换，刷新页面
            this.reload();
          }
        })
      },
