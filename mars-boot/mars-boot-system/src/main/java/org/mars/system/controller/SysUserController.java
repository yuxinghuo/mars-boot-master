...
import org.mars.system.model.DepartIdModel;
import org.mars.system.model.SysUserSysDepartModel;
import org.mars.system.service.*;
import org.mars.system.vo.SysDepartUsersVO;
import org.mars.system.vo.SysUserRoleVO;
...
public class SysUserController {
    ...
    public Result<IPage<SysUser>> queryPageList(SysUser user,
        ...
        QueryWrapper<SysUser> queryWrapper = QueryGenerator.wrapper(user);
     ...
     * 查询所选部门的用户
     */
    @GetMapping(value = "/getUserByDept")
    public Result<?> getUserByDept(@RequestParam(name = "deptId") String deptId,
                                                @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                                @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize) {
        //1.获取所属部门的用户id
        List<String> userIdList = sysUserDepartService.list(Wrappers.<SysUserDepart>lambdaQuery().eq(SysUserDepart::getDepId, deptId))
        ...
        //2.获取用户详细信息
        QueryWrapper<SysUser> queryWrapper = QueryGenerator.wrapper(new SysUser());
        ...
        // 判断总页数
        Page<SysUser> page = new Page<>(pageNo, pageSize, result.size());
        page.setRecords(ListUtil.page(pageNo-1, pageSize,result));
    ...
    public Result<?> edit(@RequestBody JSONObject jsonObject) {
        ...
        try {
            ...
            SysUser user = JSON.parseObject(jsonObject.toJSONString(), SysUser.class);
            if(sysUser.getUsername().equals(loginUser.getUsername())){
                user.setOrgCode(loginUser.getOrgCode());//设置当前登录用户所在部门
            }else{
                user.setOrgCode(""); //设置当前登录用户所在部门为空
            }
    ...
    public Result<?> changePassword(@RequestBody SysUser sysUser) {
        ...
        if (!BooleanUtil.isFalse(u.getFirstLogin())) {
    ...
    public Result<IPage<SysUser>> departUserList(@RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        ...
        if (oConvertUtils.isEmpty(depId)) {
            ...
            if (oConvertUtils.isNotEmpty(userIdentity) && userIdentity == CommonConstant.USER_IDENTITY_2) {
                subDepids = sysDepartService.getSubDepIdsByLoginUserOrgCode(user.getOrgCode());
            }
        } else {
