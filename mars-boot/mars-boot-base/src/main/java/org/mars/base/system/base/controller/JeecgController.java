package org.mars.base.system.base.controller;
...
public class JeecgController<T, S extends IService<T>> {
     ...
     * @return
     */
    protected <T> ModelAndView exportXlsByList(List<T> list, Class<T> clazz, String title) {
        LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        String realname="";
        if(StrUtil.isNotBlank(sysUser.getRealname())){
            realname=sysUser.getRealname();
        }else{
            realname=sysUser.getUsername();
        }
        // Step.3 AutoPoi 导出Excel
        ModelAndView mv = new ModelAndView(new EasypoiSingleExcelView());
        mv.addObject(NormalExcelConstants.FILE_NAME, title); //此处设置的filename无效 ,前端会重更新设置一下
        mv.addObject(NormalExcelConstants.CLASS, clazz);
        ExportParams exportParams = new ExportParams(title, "导出人:" + realname, title);
