...
<script>
  ...
  export default {
    ...
    components: {},
    props: ['modalWidth', 'multi', 'userIds','fenPage'],
    data() {
      return {
        queryParam: {
          username: "",
    ...
    methods: {
      ...
      handleSubmit() {
        ...
        console.log(that.selectUserRows, that.selectUserIds)
      ...
      getSelectUserRows(rowId) {
        if(this.fenPage){
          let  userIds = ""
            for(let j = 0;j<this.selectionRows.length;j++){
               userIds = userIds + "," + this.selectionRows[j].username
            }
           this.selectUserIds = userIds.substring(1);
          this.selectUserRows = this.selectionRows
          this.selectionRows = []
        }else{
...
// zk  2021-03-18   根据部门选择用户的时候 切换分页，选不上
      onSelectChange(selectedRowKeys, selectionRows) {
        console.log(selectedRowKeys, selectionRows)
        if(this.fenPage){
            if (selectedRowKeys.length > this.selectedRowKeys.length) {
              for (let i = 0; i < selectionRows.length; i++) {
                this.selectionRows.push(selectionRows[i])
              }
              const hash = {}
              this.selectionRows = this.selectionRows.reduce((acc, curr) => {
