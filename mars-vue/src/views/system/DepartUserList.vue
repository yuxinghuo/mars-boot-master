    ...
    <a-col :md="8" :sm="24">
      <a-card :bordered="false">
        <div style="background: #fff;padding-left:16px;height: 100%; margin-top: 5px">
          ...
          <!-- 树-->
          <!-- v-if="userIdentity === '2' && departTree.length>0" -->
          <template>

            <!--组织机构-->
            <a-tree showLine :selectedKeys="selectedKeys" :checkStrictly="true" @select="onSelect" :dropdownStyle="{maxHeight:'200px',overflow:'auto'}"
              :treeData="departTree" :loadData="onLoadData" :autoExpandParent="autoExpandParent" :expandedKeys="iExpandedKeys"
              @expand="onExpand" />
...
<script>
  ...
  export default {
    ...
    // computed:{
    //   ...mapState({
    //     department: (state) => state.utils.department,
    //   }),
    // },
    //  inject: ['reload'],
    // watch:{
    ...
    methods: {
      ...
      onSearch(value) {
        let that = this
        if(value){
           queryIdTree({name:value}).then((res) => {
            if (res.success) {
              that.departTree = []
              for (let i = 0; i < res.result.length; i++) {
                let temp = res.result[i]
      ...
      onSelect(selectedKeys, e) {
        console.log(e)
        console.log(selectedKeys, e.node.title,e.node.value)
        ...
        this.$refs.DeptUserInfo.open(record);
        // zk  2020-10-21  选中
         this.$refs.DeptUserInfo.myDepartmentTreeList = record
         console.log(this.$refs.DeptUserInfo.myDepartmentTreeList)
