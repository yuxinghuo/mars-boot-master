<template>
  <a-modal
    ...
    cancelText="取消">
    <a-tabs>
      <!-- <a-tab-pane tab="朱锟指示图标" key="0">
        <ul>
          <li v-for="icon in icons.zkList" :key="icon" @click="chooseIcon(icon)" :class="{'active':activeIndex === icon}">
            <ZK-icon :type="icon" :title="icon" />
          </li>
        </ul>
      </a-tab-pane> -->
...
// const zkList = ['zktabhome','zkiconContract','zkzhuanqian']
import ZKIcon from './zKcon.vue'
export default {
  name: 'Icons',
  props: {
    iconChooseVisible: {
