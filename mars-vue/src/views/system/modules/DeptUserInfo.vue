<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator" :md="24" :sm="24" style="margin-top: -15px">
      <!--<a-button @click="handleEdit" type="primary" icon="edit" style="margin-top: 16px">用户修改</a-button>-->
...
<script>
 ...
 import { queryMyDepartTreeList, } from '@/api/api'
  ...
  export default {
    ...
    data() {
      return {
        // zk 2020-10-15  我的部门的tree
        myDepartmentTreeList: [],
    ...
    methods: {
      ...
      handleAdd: function () {
        if (this.currentDeptId == '') {
          ...
          //zk  2020-10-15 for自调用，找到那个children 找到部门信息传过去
          // console.log(this.currentDeptId)
          // queryMyDepartTreeList(j) 
          // queryMyDepartTreeList().then((res)=>{
          //   console.log(res)
          //   if(res.success && res.result){
          //       this.findChildren(res.result)
          //         this.$refs.modalForm.resultDepartOptions = [{
          //           key: this.myDepartmentTree.key,
          //           title: this.myDepartmentTree.title,
          //         }]
          //       console.log(this.myDepartmentTree)
          //   }else{
          //       this.$message.error('未找到负责部门信息')
          //   }
          // })
          // this.$refs.modalForm.sign = true;
          console.log(this.myDepartmentTreeList)

          this.$refs.modalForm.fzbmList = [{
            key: this.myDepartmentTreeList.key,
            title: this.myDepartmentTreeList.title,
            value: this.myDepartmentTreeList.value,
          }]
      ...
      //zk  2020-10-15 for自调用，找到那个children(list)
      // findChildren(val){
      //     for(let i =0 ;i<val.length;i++){
      //         if(val[i].key === this.currentDeptId){
      //             this.myDepartmentTree = val[i]
      //             // this.forChildren(val[i])
      //           return 
      //         }else{
      //             if(val[i].children){
      //               this.findChildren(val[i].children)
      //             }
      //         }
      //     }
      // },
      //zk  list  自建一个数组传给下个页面  已删除
