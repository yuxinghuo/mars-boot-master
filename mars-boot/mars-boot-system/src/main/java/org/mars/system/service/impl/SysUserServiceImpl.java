...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    public Result<?> resetPassword(String username, String oldpassword, String newpassword, String confirmpassword) {
        ...
        this.userMapper.updateById(new SysUser()//
                .setId(user.getId())//
                .setFirstLogin(false)// 改为非首次登录
                .setPassword(password)
                .setPasswordUpdateTime(new Date()));  //记录修改密码日期
...
//			}
    ...
    @Override
    public SysUserCacheInfo getCacheUser(String username) throws BusinessException {
...
//		info.setSysUserName(user.getRealname());
        ...
        List<SysDepart> sysDept = sysDepartMapper.getDeptByOrgCode(user.getOrgCode());
        if(CollUtil.isEmpty(sysDept)){
            throw new BusinessException("部门为空");
        }
        String provoice = StrUtil.format("'{}'", sysDept.get(0).getProvinceCode());
        // 多部门支持in查询
        List<SysDepart> list = sysDepartMapper.queryUserDeparts(user.getId());
        //获取地区编码
//        List<String> dqbms = list.stream().map(SysDepart::getProvinceCode).collect(Collectors.toList());
        //将地区编码转换为逗号分隔的字符串
//        String dqbm = "'0'";
//        if (CollUtil.isNotEmpty(dqbms)) {
//            dqbm = dqbms.stream().map(d -> StrUtil.format("'{}'", d)).collect(Collectors.joining(","));
//        }
//        info.setProvinceCode(dqbm);
        info.setProvinceCode(provoice);
...
//		this.save(user);  //保存角色的时候已经添加过一次了
    ...
    @Override
    public SysDepart getCurrentDeptByUserName(String userId){
        return userMapper.getCurrentDeptByUserName(userId);
    }
