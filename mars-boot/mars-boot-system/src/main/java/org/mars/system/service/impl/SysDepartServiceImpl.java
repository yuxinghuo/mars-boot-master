...
import org.mars.system.model.SysDepartTreeModel;
import org.mars.system.service.ISysDepartService;
import org.mars.system.util.FindsDepartsChildrenUtil;
...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
    ...
    @Override
    public List<DepartIdTreeModel> queryDepartIdTreeList(QueryWrapper<SysDepart> wrapper) {
        wrapper.eq("del_flag", CommonConstant.DEL_FLAG_0.toString());
        wrapper.orderByAsc("depart_order");
        wrapper.select("id", "parent_id", "depart_name");
        List<DepartIdTreeModel> entityList = this.list(wrapper)
                .stream()
                .map(DepartIdTreeModel::new)
                .collect(Collectors.toList());
        // 调用递归树处理
        List<DepartIdTreeModel> listResult = TreeUtil.toTreeAnnoList(entityList);
        return listResult;
    }
    ...
    public List<String> getMySubDepIdsByDepId(String departIds) {
        //根据部门id获取所负责部门
        String[] codeArr = this.getMyDeptParentOrgCode(departIds);
        List<String> collect = Arrays.stream(codeArr).collect(Collectors.toList());
        return this.baseMapper.getSubDepIdsByOrgCodes(collect);
    }
    ...
    @Override
    public List<String> getSubDepIdsByLoginUserOrgCode(String orgCode){
        List<String> list=new ArrayList<>();
        list.add(orgCode);
        return this.baseMapper.getSubDepIdsByOrgCodes(list);
    }
    /**
