...
public class SysRoleController {
    ...
    public Result<IPage<SysRole>> queryPageList(SysRole role,
        ...
        QueryWrapper<SysRole> queryWrapper = QueryGenerator.wrapper(role);
    ...
    public Result<?> delete(@RequestParam(name = "id") String id) {
        int count = sysUserRoleService.count(Wrappers.<SysUserRole>lambdaQuery().eq(SysUserRole::getRoleId, id));
        if (count > 0) {
            return Result.error("已有绑定的用户，不可删除");
        }
    ...
    public Result<?> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
        ...
        int count = sysUserRoleService.count(Wrappers.<SysUserRole>lambdaQuery()
                .in(SysUserRole::getRoleId, Arrays.asList(ids.split(","))));
        if (count > 0) {
            return Result.error("已有绑定的用户，不可删除");
        }
