...
import org.mars.system.service.ISysPermissionDataRuleService;
import org.mars.system.service.ISysPermissionService;
...
public class PermissionDataAspect {
    ...
    public Object arround(ProceedingJoinPoint point) throws Throwable {
        ...
        List<SysPermission> currentSyspermission = CollUtil.toList();
        // 0.判断请求头：Referer
        String referer = ServletUtil.getHeaderIgnoreCase(request, "Referer");
        log.info("【permission】->{}", referer);
        // 截取端口之后的路径
        // http://127.0.0.1:8080/manuallyMakeInvoice/manualMakeInvoiceSpecialInvoice
        if (StrUtil.isNotBlank(referer)) {
            ...
            currentSyspermission = sysPermissionService.list(Wrappers.<SysPermission>lambdaQuery()//
                    .eq(SysPermission::getUrl, referer)//
                    .eq(SysPermission::getDelFlag, 0));
        }
        // 1.通过注解属性pageComponent 获取菜单（合并从referer中查询到的权限）
        if (StrUtil.isNotBlank(component)) {
            List<SysPermission> permissionList = sysPermissionService.list(Wrappers.<SysPermission>lambdaQuery()//
                    .eq(SysPermission::getComponent, component)//
                    .eq(SysPermission::getDelFlag, 0));
            currentSyspermission.addAll(permissionList);
        }
        // 2.通过请求路径获取权限
        if (CollUtil.isEmpty(currentSyspermission)) {
            ...
            // 1.直接通过前端请求地址查询菜单
            currentSyspermission = sysPermissionService.list(Wrappers.<SysPermission>lambdaQuery()//
                    .eq(SysPermission::getUrl, requestPath)//
            ...
            if (CollUtil.isEmpty(currentSyspermission)) {
                ...
                if (oConvertUtils.isNotEmpty(userMatchUrl)) {
                    currentSyspermission = sysPermissionService.list(Wrappers.<SysPermission>lambdaQuery()//
                            .eq(SysPermission::getUrl, userMatchUrl)//
        ...
        if (CollUtil.isNotEmpty(currentSyspermission)) {
